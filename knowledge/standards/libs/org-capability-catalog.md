---
title: org capability → canonical カタログ (lib-first)
category: libs
status: recommended
recommended: lib-first (新規実装の前に下表で canonical を引き、既存 lib があれば consume する)
alternatives: [org横断grep, その場ctags]
---

util / helper / 横断ロジックを書きたくなったら、まずこの表で canonical を探す →
無ければ org 横断検索 (`grep -rn "<機能語>" /home/user/*/src`、または その場
...
> (旧 `ippoan-lib-catalog` skill から移設。Refs ippoan/claude-skills#58)
...
| worker 認証部品 (HS256 JWT verify, timing-safe 比較, base64url, binding-jwt middleware, resolveSecret) | `@ippoan/mcp-cf-workers` の `./auth` export | consumer 移行 5/5 完了 (ippoan/mcp-cf-workers#46: cdp-relay#50 / ref-files-worker#36 / HealthConnectReaderWorker#100 / nuxt-notify#85 / secrets-inventory#75)。**dev dist-tag 運用** (`"@ippoan/mcp-cf-workers": "dev"`、MVP は release tag なし)。**自前 Web Crypto 実装を新たに書かない**。jose / agents を使わない軽量 consumer の配線 2 点: ① `.npmrc` に `legacy-peer-deps=true` (optional peer `agents` が vite>=6 要求 → vitest 2.x と ERESOLVE)、② barrel (`./auth`) でなく **subpath export (`./auth/hs256-jwt` / `./auth/crypto` / `./auth/secret`) を import** (barrel は jose 依存の cf-access / mcp-jwt を re-export するため jose 不在で tsc / vite-node が解決失敗)。実例: HealthConnectReaderWorker#100 / nuxt-notify#85 |
