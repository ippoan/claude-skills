...
| 子 (タスク) | `[S]`/`[O]` + `#c<issue>-<分岐番号> <短い題>` | `[S] #c874-12 通知先を画面から設定する` |
...
「`#p<issue>` で始まるタイトル」は常に現役の親 1 件だけを指す。
...
- **旧親が一覧に残っていても誤配しない。** 後継が起動時に旧親を
  ...
  **規約の移行中で、旧形式のタイトルを prompt に持つ子が走っている間だけ**は、
  1 回だけ「新しい判別条件は `#p<issue>` で始まるタイトル」を通知する。

## 2. 分割の原則

- **名前は `#c<issue>-<分岐番号> <短い題>`。** spawn_task の title に必ずこの形で付ける
  (例: 「#c205-1 fold の読み分離と unnest 化」)。分岐番号は依存順 (= マージ順) と
