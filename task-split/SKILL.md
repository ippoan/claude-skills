...
4. テストの期待と、**「branch push まで。PR は親の『PR 作成 go』が来てから ready で
   作る」** の指示 (交通整理の節)。Refs する issue 番号。DB 相手のテストがあるなら
...
- **PR の順番は親が統制する — 栓は「PR 作成のタイミング」** (ユーザー指示 2026-07-31)。
  ...
  親が依存順に「PR 作成 go」を出し、子 (セッション終了済みなら親) が PR を
  **ready で**作成 → CI 1 回 → auto-merge が green で即 merge。後発には先行の
...
- **archive の主役は「Auto-archive on PR close」設定** (ユーザーがアプリの
  Settings で有効化)。ただし**取りこぼす条件がある** — セッションに紐づく
  `prNumber` が**起動 repo と別 repo の PR**だと拾われない (実害 2026-07-31:
  2 repo にまたがるタスクが畳まれ残った)。
  `archive_session` は self 含め **毎回ユーザー承認プロンプトが出る**
  (permission mode に関係なく必須)。
- **子は最終 [完了] に「archive 可否」の確認を 1 行添える** ([[report-to-parent]] と対)。
  親は**基準 3 点を裏取りして**可否を返す — ①PR が MERGED (**全部の repo で**)、
  ②掃除完了 (worktree / コンテナ / local+remote branch)、③未消化の申し送り無し
  (あるならチップ化済み)。可なら子が self-archive する。
  **畳むタイミングを握るのは子のまま** (親が勝手に畳んで調査続行中の子を止めた
  事故があるため)。親が保証するのは条件だけ。
...
- **親が畳むのは取りこぼしだけ** (自畳みせず終了通知だけ来たセッション等)。
  その場合も **[完了] 受領だけで畳まない** — 子は [完了] 後に自発的な精査を
  続けることがある (実害 2026-07-31: キー衝突の調査続行中に親が archive して
  中断させた)。畳む前に list_sessions の `lastActivityAt` で活動停止を確認するか、
  「畳んでよいか」を一言送って返答を待つ。
