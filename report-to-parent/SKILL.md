...
**親のタイトルは `#p<issue> <短い題>`、自分 (子) は `#c<issue>-<分岐番号> <短い題>`**
という規約 ([[task-split]] の命名規約)。`#p` と `#c` で親子が分かり、`<issue>` が
一致するものだけが同じ案件。

1. 起動 prompt に親のタイトルが書いてある →
   `mcp__ccd_session_mgmt__list_sessions` を呼び、タイトル一致の `sessionId` を取る。
   完全一致が無ければ **`#p<issue>` で始まるセッション**を探す (`<issue>` は自分の
   `#c<issue>-…` から取れる)。
...
- **PR に紐づくセッション**: 最終 [完了] に**下の「archive 可否」1 行を必ず添えて**
  送る。ユーザーの「Auto-archive on PR close」設定が merge 時に畳んでくれれば
  それで終わり。畳まれなかったときのために親の確認を取っておく。
- **`prNumber` が起動 repo と違う PR で終わるタスクは、auto-archive が拾わない。**
  実害 2026-07-31: 2 repo にまたがるタスクでセッションに紐づいた `prNumber` が
  別 repo の PR だったため畳まれ残った。**2 repo 以上に PR を出したなら、
  自分で畳む前提で動くこと。**

### archive 可否の確認 (最終 [完了] に 1 行添える)

> archive 可否: PR は `<owner>/<repo>#<番号>` (MERGED) / 掃除済み (worktree・
> branch・コンテナ) / 未消化の申し送り無し。畳んでよいですか

親が 3 条件を裏取りして「畳んでよい」と返したら、`archive_session` を
`session_id: "self"` で呼ぶ (ユーザー承認プロンプトが出る)。
**畳むタイミングを握るのは子のまま** — 親が勝手に畳んで調査続行中の子を止めた
事故があるため。親が保証するのは「条件を満たしたか」だけ。
親から返事が来なくても、3 条件を自分で満たせているなら畳んでよい。
- **PR を作らないセッション** (検証・調査系) だけ、下の self-archive を使う。

## 自分で畳む (PR 無しのセッションのみ)

PR の merge・掃除 (worktree / コンテナ / branch)・申し送りのチップ化まで**全部**
終わり、追加でやることが本当に無くなったら、最終 [完了] を送ってから
`archive_session` を `session_id: "self"` で呼んで自分のセッションを畳む
(確認プロンプトが出て、その tool result で会話が終わる)。親に畳ませると
「まだ精査中なのに止められる」事故が起きるので、畳むタイミングは自分で決める。
まだ調べたいことが残っているなら畳まずに続けてよい — その場合は一言
[報告] を親に送っておく。
