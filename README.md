...
- **report-to-parent** — spawn_task で起動された**子側**が、起動元 (親) へ `send_message` で報告する運用。着手時 / 設計判断に迷ったとき / branch push 完了時。親の見つけ方 (タイトル逆引き) と、**子は self-archive せず親の archive を待つ**規約を含む。
