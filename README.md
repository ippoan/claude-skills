...
- **ippoan-lib-catalog** — org の「この機能の canonical 実装はどこか」の capability 粒度カタログ。util / helper / 横断ロジックを新規実装する前に参照し、既存 lib があれば consume する (lib-first)。rule of two / SOURCE-MIRROR 規約 / 監査記録 (ippoan/claude-md#76) への pointer も。
