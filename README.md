...
- **wrangler-deploy-temporary** — `wrangler deploy --temporary` (Wrangler 4.102.0+) で認証不要の一時 Cloudflare アカウントに実 Workers edge へ即デプロイし、60分で自動失効する PoC ハーネス。外部サイトへの Workers egress 到達性検証 (WAF/datacenter IP ブロックの有無、実 colo 確認) に使う。デプロイ結果 URL は Cloudflare の JS challenge に守られるため curl では読めず、cdp-relay 等の実ブラウザ経由で読む。
...
- **browser-cookie-borrow** — CCoW から credential login すると Anthropic egress gateway が TLS を MITM 終端して credential を平文復号する (実測: 外部サイトの証明書 issuer が `O=Anthropic, CN=Egress Gateway SDS Issuing CA`) 問題を回避し、**login だけ手元ブラウザに委譲**して cdp-relay `browser_cookies` (CDP `Network.getCookies`、HttpOnly も取れる) で cookie を借り、**検索/CSV 等の認証後操作だけ CCoW 内で高速反復**するパターン。credential は手元 egress だけを通り CCoW / gateway を通らない。scraper の cookie jar が login と分離済み (login skip entry) が前提。cookie は stash 経由・origin 絞り・opt-in。login 実装自体は未検証と正直に扱う。ETC (etc-meisai) の worked example。bun-browser-verify の cookie 特化版。
