...
- **pr-chat-bridge** — PR コメントを transport に CCoW セッションと Claude chat (Desktop / Cowork + Claude in Chrome) を連携させるブリッジ。CCoW がチェックリスト付き依頼コメントを投稿 → user が chat 起動リンク (`claude.ai/new?q=…` プリフィル) をクリック → chat がブラウザ検証して結果コメント → webhook で CCoW が起床して処理。**auto-merge 既定の org なので draft PR (preview 導入 repo 限定、staging は draft で deploy されない) / merge 後 issue + send_later 回収の分岐**が肝。Refs #102。
