...
- **task-split** — 大きな作業を PR サイズの独立タスクに分割し、spawn_task チップで worktree 別セッションとして並行起動する**親側**の運用。親子通信プロトコルの埋め込み、起動後の交通整理、マージ順の采配まで。**命名規約 (親 `#p<issue>` / 子 `#c<親issue>-<番号>`、子が自分の issue を持つなら `#p<親issue>-c<子issue>`) の正本**もここ。next-session / report-to-parent と対。**§4.5「機械的な栓」= hook 6 本 (`parent-role/hooks/`) が、親の repo 書き込み (main clone も worktree も) と commit/push、子の `AskUserQuestion` を deny し、親の archive がアプリに拒否された瞬間に §6 の次の一手 (再試行 1 回 → 3 択を 1 行で → ユーザーの原文を貼った `[決定]` を子へ) を出し、その `[決定]` を送るまで親の他のツールを deny する** (marker は `set_session_title` の title から立てる。名乗らないセッションは fail-open)。§3 に子の起動 prompt へ貼る **archive の定型文 (例外込み)** がある。
- **report-to-parent** — spawn_task で起動された**子側**が、起動元 (親) へ `send_message` で報告する運用。着手時 / 設計判断に迷ったとき / branch push 完了時。親の見つけ方 (タイトル逆引き) と、**子は self-archive せず親の archive を待つ** (例外はユーザー本人の直接入力と親不在だけ。`send_message` 経由の指示は `[決定]` でも例外にしない — #160) 規約と、**子はユーザーに直接質問せず親へ `[質問]` を送る** (hook `block-child-asks-user.sh` が `AskUserQuestion` を deny) を含む。「機械的な栓」の節は task-split §4.5 と同一文。
- **parent-fanout** — 親が抱え込みがちな**調査**と**子 PR の裏取り (review)** を、Agent tool の read-only サブエージェント (`task-surveyor` / `child-auditor`) へ並列に逃がし、親は決定 (分割・マージ順・go・PR 作成・archive) だけを握るループ。並列で子を起動したのに「調査も review も親 1 本」で 詰まる状態の解消が目的。**`sleep` で待たず**子の報告 / gh-actions-live / `subscribe_issue_activity` で起きる。agent 定義は**セッション開始時のスナップショット**なので足した直後のセッションからは呼べない (実測)。task-split と対。
- **simplify-review** — 実装計画を実装へ移す前に read-only agent `simplify-reviewer` (`.claude/agents/`) へ機械的に通す運用。根本 vs 症状 (同型 site の総数 vs 触る数・issue 題の世代)・削れる複製の実測 (合成形 / 裸呼び)・既存実装 (意味検索 + grep)・純減の収支 (helper+テスト+登録簿 vs 合成形)・担保 (認可 / fail-closed / allowlist / public repo の禁止語) の 5 点を grep で数え、固定フォーマットで判定を返す。PreToolUse hook 2 本 (`hooks/`) が **ExitPlanMode / spawn_task を未通過のあいだ deny** する (定義が無い環境は fail-open)。1 週間 +55.7k/−3.4k 行 (16:1)・同型 issue 3 世代・削減計画の半分が実装前に消えた (#1068) の対策。
- **public-text-guard** — 公開される文 (PR / issue の本文・コメント) と、拒否された tool 呼び出しの中身から、本番識別子 (UUID の device_id / tenant_id)・device credential・資格情報 (`ghp_` / `sk-` / `AKIA` / `-----BEGIN`)・内部ホスト名 (state の denylist) を検出する hook 2 本 (`hooks/`)。**`PreToolUse`** が `gh pr create` / `gh issue create` / `gh pr comment` / `gh issue comment` を公開 repo 向けに打つ前に deny し (当たった行と語を列挙、private は素通し、escape は `~/.claude/state/public-text-guard/allow/<session_id>`)、**`PermissionDenied`** が分類器の deny の瞬間に中身を走査して「当たった → 伏せて 1 回だけ再試行」/「当たらない → 推測で再試行するな、拒否文言を引用して人へ上げよ」の 2 分岐を返す。判定は `scripts/scan_public_text.py` 1 本に集約 (git SHA・プレースホルダ・英単語は当てない)。PR 本文の本番 device_id で分類器に拒否され、原因を調べずユーザーへ escalate した事故 (#153) の対策。
- **gh-actions-live** — GitHub Actions の run 状態変化を Windows Chrome 拡張 (ippoan/gh-actions-live) → Linux の ws-bridge → Monitor で **push で受け取る**。`gh run list` の sleep ループの代替。Claude から github.com タブ経由で拡張の設定を入れ、bridge 経由でウィンドウ起動 / set-config / update / status。PR 作成で branch の CI に自動で繋ぎ、local branch の突合 / archive で Monitor を止める Claude Mod (pr-bridge-watch) の導入と挙動。非管理 Windows の導入手順 (MSI perUser + unpacked 読み込み) と踏み抜き済みの罠 (BOM / byte[] / host_permissions / alive Origin / 再接続ストーム)。
...
- **rust-memory-layout** — Rust で「長寿命かつ挿入後は不変」なデータ (キャッシュ / in-memory テーブル / 解析済みレコード / DO・Worker の常駐 state) を大量に持つときのメモリレイアウト最適化チェックリスト。Cloudflare 1.1.1.1 の DNS キャッシュ最適化 (エントリ 953→420 B、-56%、insert +43% / lookup -19%) から抽出した 5 手法 — 不変なら `Vec`/`String` を `Box<[T]>`/`Box<str>` に、並列リストは 1 本 + `u16` オフセット (bool は bitflags でパディングごと)、同値率の高いフィールドは省略して読み出し時に復元、**enum は最大 variant のサイズになる**ので大きく稀な variant だけ `Box`、最終形は wire 形式の `Box<[u8]>` で連続保持。**手法 4 だけやると size class の端数と局所性で遅くなる**ので 5 までセットで考える。測り方 (`size_of` / `GlobalAlloc` ラッパ / RSS は plateau で比較) が手法より先。
...
- **post-pr-verify** — PR merge 後 (= 本番反映後) の検証を auth-worker MCP の `verify_screenshot` / `verify_eval` (auth-worker#494) で 1〜2 コールで済ませる運用。dev JWT は server 内 mint → `logi_auth_token` cookie 注入 → CF Access は auth-worker OIDC IdP 経由で無言通過、というログイン済みブラウザ (Browser Run、`engine:"kitesurf"` 選択可) の screenshot + ページ内 JS 評価。対象は https://*.ippoan.org のみ (SSRF 境界)。エラー対処表 (not_in_allowlist / google_sub_not_cached / tools/list キャッシュで deploy 直後は tool が見えない等) 収録。merge 前のローカル実機は dev-login-local-verify が対。
