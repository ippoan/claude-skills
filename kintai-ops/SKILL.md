...
スナップショットの中を検索するだけですが、**セッション開始時から設定済みの connector は
接続が終わると遅れて一覧に現れます (実測 2026-09-12)。1 回見えないだけで「無い」と決めない**
([[parent-fanout]] の罠)。**自分で足した tool は自分では呼べません。** 見えない tool が
あったらユーザーに connector 追加を頼むのが最短です (走行中のセッションにも入ります)。
