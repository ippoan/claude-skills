...
2. **後継は起動したら、まず名前を入れ替える。順序厳守**
   ...
   2. **続けて自分**を `set_session_title { session_id: "self" }` で
      `#p<issue> <短い題>` へ改名
   逆順にすると `#p<issue>` で始まるセッションが一瞬 2 件になり、その隙に届いた
...
そのまま新しい親を指す。**例外は規約の移行中** — 旧形式のタイトルを prompt に
持つ子が走っているときだけ、1 回「新しい判別条件は `#p<issue>` で始まるタイトル」
を送る。
...
> 報告は `list_sessions` でタイトル逆引き。宛先は **`#p\<issue\>` で始まる**
> セッション (`[旧]` が付くものは交代前の旧親なので送らない)。
